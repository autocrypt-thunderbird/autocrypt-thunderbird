...
function sendMessage(aParams, {
  ...
  switch (compType) {
    ...
    case mCompType.Draft:
      {
        ...
        // about to send...
        if (urls.length != 1) {
          EnigmailLog.ERROR("Can't edit more than one message at a time\n");
        }
    ...
    case mCompType.ReplyToList:
      {
        if (urls.length != 1) {
          EnigmailLog.ERROR("Can't reply to more than one message at a time\n");
        }
        let msgHdr = msgUriToMsgHdr(urls[0]);
        references = [...range(0, msgHdr.numReferences)].map(i => msgHdr.getStringReference(i));
    ...
    case mCompType.ForwardInline:
      {
        if (urls.length != 1) {
          EnigmailLog.ERROR("Can't forward inline more than one message at a time\n");
        }
        let msgHdr = msgUriToMsgHdr(urls[0]);
        references.push(msgHdr.messageId);
        break;
  ...
  let defaultFcc =
    fccFolder.indexOf("nocopy://") == 0 ? "" : fccFolder;
  ...
  if (archive) {
    ...
    if (MailUtils.getFolderForURI(folderUri, true)) {
      ...
      EnigmailLog.DEBUG("Message will be copied in " + folderUri.toString() + " once send\n");
      fields.fcc = folderUri;
    }
    else {
      EnigmailLog.WARNING("The archive folder doesn't exist yet, so the last message you sent won't be archived... sorry!\n");
  ...
  if (popOut) {
    ...
    params.format = composeHtml ? Ci.nsIMsgCompFormat.HTML : Ci.nsIMsgCompFormat.PlainText;
    ...
    aBody.match({
      plainText: function(body) {
        ...
        fields.body = body; //simpleWrap(body.replace(/ +$/gm, ""), 72)+"\n";
        ...
        //gMsgCompose.compFields.setRawHeader("Content-Type","multipart/encrypted; protocol=\"application/pgp-encrypted\";\n\tboundary=\"=-=01-ud6u17mc5xyf6ikhwmqo=-=\"");
      },
      ...
      editor: function(iframe) {
        ...
        try {
          ...
          EnigmailLog.DEBUG(e.toString());
          gMsgCompose.editor = getEditorForIframe(iframe);
        }
        let convertibility = gMsgCompose.bodyConvertible();
        EnigmailLog.DEBUG("This message might be convertible: " + convertibility.toString() + "\n");
