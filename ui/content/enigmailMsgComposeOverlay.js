...
ChromeUtils.import("chrome://enigmail/content/modules/wkdLookup.jsm"); /*global EnigmailWkdLookup: false */
...
Enigmail.msg = {
  ...
  composeStartup: function() {
    ...
    function loadOverlay(targetWindow, srcUrl) {
      let {Overlays} = ChromeUtils.import("chrome://enigmail/content/modules/overlays.jsm", {});
  ...
  */
  ...
  encryptMsg: function(msgSendType) {
    ...
    let {sendFlags, gotSendFlags} = this.getEncryptionFlags(msgSendType);
      ...
      if ("errArray" in missingKeys && missingKeys.errArray.length > 0) {
        ...
        if (lookupList.length > 0) {
          new Promise((resolve, reject) => {
            ...
            EnigmailWkdLookup.findKeys(lookupList).then((foundKeys) => {
