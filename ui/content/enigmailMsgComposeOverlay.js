...
Enigmail.msg = {
  ...
  */
  processFinalState: function(sendFlags) {
    ...
    EnigmailLog.DEBUG("enigmailMsgComposeOverlay.js:   encrypt=" + ((this.sendMode & ENCRYPT) !== 0) + " encFinally=" + encFinally + "\n");
    ...
    EnigmailLog.DEBUG("enigmailMsgComposeOverlay.js:   signed=" + ((this.sendMode & SIGN) !== 0) + " signFinally=" + signFinally + "\n");
    ...
    EnigmailLog.DEBUG("enigmailMsgComposeOverlay.js:   pgpmimeFinally=" + pgpmimeFinally + "\n");
  ...
  setDraftStatus: function(doEncrypt) {
    ...
    var draftStatus = "N" + this.encryptForced + this.signForced + this.pgpmimeForced +
      "0" + (doEncrypt && this.protectHeaders ? "1" : "0");
   ...
   */
  keySelection: function(enigmailSvc, fromAddr, toAddrList, bccAddrList) {
  ...
  encryptMsg: async function(msgSendType) {
    ...
    try {
      ...
      let encodedPrivKey = result.encodedPrivKey;
      let encodedPubKeys = result.encodedPubKeys;
      ...
      if ((sendFlags & (ENCRYPT | SIGN)) && usingPGPMime) {
        ...
        newSecurityInfo.encodedPrivKey = encodedPrivKey;
        newSecurityInfo.encodedPubKeys = encodedPubKeys;
   ...
   */
  handleSendMessageEvent: async function(event) {
    EnigmailLog.DEBUG("enigmailMsgComposeOverlay.js: Enigmail.msg.handleSendMessageEvent I AM ASYNC\n");
    ...
    if (!(this.sendProcess && sendMsgType == Components.interfaces.nsIMsgCompDeliverMode.AutoSaveAsDraft)) {
      ...
      try {
        ...
        let encryptResult = await this.encryptMsg(sendMsgType);
        if (!encryptResult) {
